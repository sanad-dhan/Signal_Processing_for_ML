%% MFCC Dictionary + Distance Matching (Euclidean, Manhattan, Cosine + Full Test)
close all;
clc; 
clear all;
%% LOAD DATA 
ads=audioDatastore('E:\Sanad sem 8\SP4ML\Project\speech_data', ...
...
%% CONFIGURATION 
numStudents=100;
numFilesPerStudent=10;   % files used to BUILD dictionary
testStartIdx=11;   % files from this index onwards used for testing
featureSize=14;   % 13 MFCC + 1 volume
numIterations=1000;
...
for i=1:numStudents
    ...
    for j=1:n
        ...
        tempmat(:, j)=mean(coeffs, 1)';
    end
    D(:, i)=mean(tempmat, 2);
end
fprintf('Dictionary built: %d x %d\n', size(D));
%% 1000-TRIAL TEST (Euclidean, Cosine, Manhattan)
correctEuc=0;
correctCos=0;
correctMan=0;
for i=1:numIterations
    ...
    testCoeffs=mfcc(testAudio, testFs);
    y=mean(testCoeffs, 1)';
...
% Every file beyond numFilesPerStudent per speaker is a test file
...
for i=1:nTest
    ...
    coeffs=mfcc(audio, fs);
    Y_test(:, i)=mean(coeffs, 1)';
end
% Evaluate all three metrics on full test set
